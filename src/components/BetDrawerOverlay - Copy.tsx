    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          // VS line achievement (OVER/UNDER vs the line)
          const vv = Number(d?.v ?? 0);
          const overUnder = Number.isFinite(vv) && vv > lineValue ? "OVER" : "UNDER";

          // dot color depends on selected pick side (hit)
          const dot = d?.hit ? "rgba(34,197,94,0.98)" : "rgba(239,68,68,0.98)";

          const achievement = `${fmtLine(lineValue)} ${String(propShortLabel ?? "").trim() || "—"}`;
          ...
          return `
            <div style="display:flex;flex-direction:column;gap:4px;min-width:140px">
              ...
              <div style="font-size:11px;opacity:0.78;font-weight:800">VS LINE: <span style="opacity:0.98">${overUnder}</span> <span style="opacity:0.62">${achievement}</span></div>
            </div>
          `;
        },
      },
