...
import { resolveEuroleagueDisplayName, resolveEuroleagueLogoSlug, resolveEuroleagueLogoUrl } from "@/lib/teamLogos";
...
function PropCategoryDropdown() {
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const left = Math.max(8, Math.min(r.left, window.innerWidth - 328)); // keep inside viewport
    ...
    // capture scroll on window + any scrollable parents
  ...
  function toggleAll() {
    s.set("propCategories", []); // empty => ALL
  }
...
function PlayerDropdown() {
  ...
  const selectedPlayers = useFilters((x) => (x as any).selectedPlayers) as string[] | undefined;
  const selectedTeam = useFilters((x) => (x as any).selectedTeam) as string | null | undefined;
  const search = useFilters((x) => (x as any).playerSearch) as string | undefined;
  ...
  const allActive = (!selectedPlayers || selectedPlayers.length === 0) && !selectedTeam;
  ...
  const presentTeams = useMemo(() => {
    ...
    if (canonical.length) {
      const out = canonical
        ...
        // alphabetical by display name (surname-style expectation: stable, A–Z)
        .sort((a, b) => a.display.localeCompare(b.display, undefined, { sensitivity: "base" }));

      return out;
    }

    // fallback: derive from whatever teams are present in the feed
  ...
  const filteredPlayers = useMemo(() => {
    ...
    const teamKey = selectedTeam ? String(selectedTeam) : null;

    const getPlayerTeamKey = (p: any): string | null => {
      if (p?.teamKey) return String(p.teamKey);
      // Back-compat: allow team key to be encoded in the option key prefix "TEAM::RAW_NAME"
      if (typeof p?.key === "string" && p.key.includes("::")) {
        return String(p.key.split("::")[0] || "");
      }
      return null;
    };
    ...
    const arr = (players || [])
      .filter((p) => {
        if (teamKey) {
          const pk = getPlayerTeamKey(p);
          if (!pk || pk !== teamKey) return false;
        }
        if (!q) return true;
  ...
  function pickTeam(slug: string) {
    ...
    s.set("selectedPlayers" as any, []); // keep semantics: ALL PLAYERS (within team)
    s.set("playerSearch" as any, ""); // ensure A–Z list is visible after team selection
    setShowTeams(false);
  }
  ...
  const summary = useMemo(() => {
    ...
    if (selectedPlayers && selectedPlayers.length === 1) {
      ...
      return (p?.surname && p?.name) ? `${p.surname} ${p.name}` : (p?.surname ?? p?.name ?? "1 selected");
    }
    if (selectedPlayers && selectedPlayers.length > 1) return `${selectedPlayers.length} selected`;
    if (selectedTeam) {
      const d = resolveEuroleagueDisplayName(selectedTeam) ?? selectedTeam;
      const short = String(d)
        .trim()
        .split(/\s+/)[0] || String(selectedTeam).split("_")[0] || String(d);
...
<div className="grid grid-cols-4 gap-2">
                    ...
                    <div className="mt-2 text-[11px] text-white/55">
                      Tip: selecting a team shows only players from that team with available props.
                    </div>
                  </div>
                ) : null}
              </div>
              ...
              <div className="px-2">
                ...
                <Input
                  value={search ?? ""}
                  onChange={(e) => s.set("playerSearch" as any, e.target.value)}
                  className="w-full"
              ...
              {!String(search ?? "").trim() ? (
                ...
                {(!players || players.length === 0) ? (
                  ...
                  <div className="px-3 py-2 text-[12px] text-white/55">
                    No players for the selected team.
                  </div>
                ) : null)}
                  </div>
                </>
...
export function TopBar() {
  ...
  const [matchOptions, setMatchOptions] = useState<MatchOption[]>([
    { label: "MATCH", value: "all" },
  ]);

  // ---- keep ASC/DESC positioned right under "Last N" (slightly to the left)
  const innerRef = useRef<HTMLDivElement>(null);
  const row1ScrollRef = useRef<HTMLDivElement>(null);
  const lastNRef = useRef<HTMLDivElement>(null);
  const [ascLeft, setAscLeft] = useState<number>(0);

  useLayoutEffect(() => {
    const update = () => {
      const inner = innerRef.current;
      const lastN = lastNRef.current;
      if (!inner || !lastN) return;

      const innerRect = inner.getBoundingClientRect();
      const lastRect = lastN.getBoundingClientRect();

      // "λίγο πιο αριστερά" => small nudge left
      const left = Math.max(0, lastRect.left - innerRect.left - 6);
      setAscLeft(left);
    };
    ...
    const scroller = row1ScrollRef.current;
    ...
    fetch(`${API_BASE}/api/upcoming-matches${qs}`)
      ...
      .then((data) => {
        ...
        if (Array.isArray(data)) {
          setMatchOptions([{ label: "MATCH", value: "all" }, ...data]);
        } else {
          setMatchOptions([{ label: "MATCH", value: "all" }]);
        }
      })
      .catch(() => {
        if (!alive) return;
  ...
  const safeMatchOptions = useMemo(() => {
    ...
    const exists = matchOptions.some((o) => o.value === curr);
    if (exists) return matchOptions;
    return [{ label: "MATCH", value: curr }, ...matchOptions];
  }, [matchOptions, s.match]);

  return (
    <div className="sticky top-0 z-20 border-b border-stroke bg-bg/80 backdrop-blur-xl">
      <div ref={innerRef} className="mx-auto max-w-6xl px-4 py-3">
        <div className="flex flex-col gap-2">
          {/* Row 1 */}
          <div
            ref={row1ScrollRef}
            className="flex flex-nowrap items-center gap-2 overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch]"
          >
            <Select
              value={s.match}
            ...
            {/* anchor for ASC/DESC on row 2 */}
            <div ref={lastNRef} className="flex items-center gap-2">
              <button
                type="button"
          ...
          <div className="relative min-h-[40px]">
            <div className="flex flex-nowrap items-center gap-2 overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch]">
              <PropCategoryDropdown />
              <PlayerDropdown />
            </div>

            {/* ASC/DESC placed right under the "Last N" group */}
            <div
