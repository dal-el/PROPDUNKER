...
function selectMainForCategory(opts: {
  ...
  const wantedPropKey = (() => {
    ...
    if (t === "TD" || t === "TRIPLED0UBLE" || t === "TRIPLEDOUBLE") return "TD";
    ...
    // OTHER
    if (t === "FOULS") return "F";
    if (t === "FOULSD") return "FD"; // UI label: "FOULS D"

    // 1st period (Q1)
    if (t === "1PPOINTS") return "Q1_PTS";
    if (t === "1PREBOUNDS") return "Q1_TR";
    if (t === "1PASSISTS") return "Q1_AS";
    if (t === "1P3PM") return "Q1_3P_M";
...
function CinematicEChartsBar({
  ...
  const option = React.useMemo(() => {
    ...
    const logoSize = barsCount >= 20 ? 18 : barsCount >= 15 ? 22 : 28;
    ...
    return {
      ...
      tooltip: {
        ...
        extraCssText:
          "border-radius:14px; box-shadow:0 24px 70px rgba(0,0,0,0.65); backdrop-filter:blur(14px); padding:10px 12px;",
        formatter: (params: any) => {
          const p = Array.isArray(params) ? params[0] : params;
          const idx = p?.dataIndex ?? 0;
          const d = orderedChartData[idx];
          const tag = d?.over ? "OVER" : "UNDER";
          return `<div style="display:flex;gap:10px;align-items:center;">
            <span style="width:10px;height:10px;border-radius:999px;background:${d?.hit ? "rgba(34,197,94,0.98)" : "rgba(239,68,68,0.98)"};box-shadow:0 0 0 3px rgba(255,255,255,0.08)"></span>
            <div>
              <div style="font-size:12px;opacity:0.9;font-weight:800">
                ${d?.label ?? "—"}: <span style="color:#fff">${(() => {
                  const vv = Number(d?.v ?? 0);
                  if (!Number.isFinite(vv)) return 0;
                  const rounded = projectionEnabled ? Math.round(vv * 10) / 10 : vv;
                  // show up to 1 decimal, but trim .0 when not projecting
                  const s = projectionEnabled ? rounded.toFixed(1) : (Number.isInteger(rounded) ? String(rounded) : (Math.round(rounded*10)/10).toFixed(1));
                  return projectionEnabled ? s : s.replace(/\.0$/, "");
                })()}</span>
                <span style="opacity:0.65;margin-left:6px">vs ${fmtLine(lineValue)}</span>
                <span style="opacity:0.65;margin-left:6px">${(() => {
                  const mm = Number(d?.minutes);
                  return Number.isFinite(mm) ? `MIN ${Math.round(mm)}` : "MIN —";
                })()}</span>
              </div>
              <div style="font-size:11px;opacity:0.65;font-weight:800;margin-top:4px">${tag}${(typeof d?.minutes === "number" && Number.isFinite(d.minutes)) ? ` • MIN ${Math.round(d.minutes)}` : ""}</div>
            </div>
          </div>`;
        },
      },
  ...
  }, [orderedChartData, lineValue, maxV, barWidth]);
                  ...
                  return (
                    <button
                      ...
                      className={clsx(
                        ...
                        active
                          ? "border-white/60 bg-white/10 text-white shadow-[0_0_0_1px_rgba(255,255,255,0.25)]"
